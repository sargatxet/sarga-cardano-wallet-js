...
export class MultisigTransaction {
  ...
  adjustFee(
    ...
    if (txFee < bodyFee) {
      ...
      outputs.push(
        ...change.map((ch) => {
          const address = Address.from_bech32(ch.address);
          const amount = Value.new(
          ...
          if (ch.assets && ch.assets.length > 0) {
            const multiAsset = Seed.buildMultiAssets(ch.assets, encoding);
            amount.set_multiasset(multiAsset);
          }
  ...
  private txFee(tx: Transaction, config: any) {
    return parseInt(
      min_fee(
        tx,
        LinearFee.new(
          BigNum.from_str(config.protocolParams.txFeePerByte.toString()),
          BigNum.from_str(config.protocolParams.txFeeFixed.toString())
        )
      ).to_str()
    );
  }
